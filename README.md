...
1. As a `github action` ([_» see all github action settings_](#github-action)):
   ```yaml
   uses: antitoxic/prerender-spa-ultra@v1
   with:
     website_root: 'path/to/your/spa/dist'
   ```
2. As `CLI` ([_» see all cli arguments_](#cli)):
   ...
   [via cli args](#github-action).

3. As a `npm` package ([_» see full config_](#npm-package)):
   > _In contrast to the other approaches, when using programmatically you are
   > responsible for starting the http server for your static files.
   > [» See examples](#http-server-with-fallback) how you can do this._
   ```typescript
...
1. using a python 3 file included in `prerender-spa-ultra`
   ...
   python <path/to/node_modules>/prerender-spa-ultra/src/http-server.py
   ```
1. using the `node-static` package
   ```shell
   npx node-static --spa
